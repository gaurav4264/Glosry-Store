...
async function fallback() {
    ...
    // EXACT known images for specific products
    const exactMatchMap = {
    ...
    for (let p of products) {
        ...
        // Find if any exact match keys exist in the product name
        ...
        if (!p.image || p.image.length === 0 || p.image[0] !== bestImg) {
            p.image = [bestImg];
            await p.save();
            count++;
        }
    ...
    console.log(`Fallback exact/category matches applied to ${count} products.`);
    process.exit(0);
}
